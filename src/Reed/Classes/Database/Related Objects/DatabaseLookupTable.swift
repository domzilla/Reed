...
final class DatabaseLookupTable: Sendable {
    ...
    init(
        name: String,
        objectIDKey: String,
        relatedObjectIDKey: String,
        relatedTable: DatabaseRelatedObjectsTable,
    ...
    func fetchRelatedObjects(for objectIDs: Set<String>, in database: FMDatabase) -> RelatedObjectsMap? {
        ...
        if let relatedObjects = fetchRelatedObjectsWithIDs(relatedObjectIDsMap.relatedObjectIDs(), database) {
            let relatedObjectsMap = RelatedObjectsMap(
                relatedObjects: relatedObjects,
                relatedObjectIDsMap: relatedObjectIDsMap
            )

            let objectIDsWithNoFetchedRelatedObjects = objectIDsThatMayHaveRelatedObjects
                .subtracting(relatedObjectsMap.objectIDs())
    ...
    func saveRelatedObjects(for objects: [DatabaseObject], in database: FMDatabase) {
...
extension DatabaseLookupTable {
    // MARK: Removing

    private func removeRelationships(for objects: [DatabaseObject], _ database: FMDatabase) {
        let objectIDs = objects.databaseIDs()
        let objectIDsToRemove = self.objectIDsSubtractingObjectIDsWithNoRelatedObjects(objectIDs)
        if objectIDsToRemove.isEmpty {
            return
        }

        database.rd_deleteRowsWhereKey(self.objectIDKey, inValues: Array(objectIDsToRemove), tableName: self.name)
    }

    private func deleteLookups(for objectID: String, _ relatedObjectIDs: Set<String>, _ database: FMDatabase) {
        ...
        let placeholders = NSString.rd_SQLValueList(withPlaceholders: UInt(relatedObjectIDs.count))!
    ...
    private func updateRelationships(for objects: [DatabaseObject], _ database: FMDatabase) {
        if objects.isEmpty {
            return
        }
        ...
        if let lookupTable = fetchRelatedObjectIDsMap(objects.databaseIDs(), database) {
            for object in objects {
                self.syncRelatedObjectsAndLookupTable(object, lookupTable, database)
            }
        }

        // Save the actual related objects.

        let relatedObjectsToSave = self.uniqueArrayOfRelatedObjects(with: objects)
        if relatedObjectsToSave.isEmpty {
            assertionFailure(
                "updateRelationships: expected relatedObjectsToSave would not be empty. This should be unreachable."
            )
            return
        }

        self.relatedTable.save(relatedObjectsToSave, in: database)
    }

    private func uniqueArrayOfRelatedObjects(with objects: [DatabaseObject]) -> [DatabaseObject] {
        // Can’t create a Set, because we can’t make a Set<DatabaseObject>, because protocol-conforming objects can’t be
        // made Hashable or even Equatable.
        // We still want the array to include only one copy of each object, but we have to do it the slow way.
        // Instruments will tell us if this is a performance problem.
    ...
    private func syncRelatedObjectsAndLookupTable(
        ...
        if !relatedObjectIDsToDelete.isEmpty {
            self.deleteLookups(for: object.databaseID, relatedObjectIDsToDelete, database)
        }

        let relatedObjectIDsToSave = relatedObjectIDs.subtracting(lookupTableRelatedObjectIDs)
        if !relatedObjectIDsToSave.isEmpty {
            self.saveLookups(for: object.databaseID, relatedObjectIDsToSave, database)
        }
    }

    private func saveLookups(for objectID: String, _ relatedObjectIDs: Set<String>, _ database: FMDatabase) {
        for relatedObjectID in relatedObjectIDs {
            let d: [NSObject: Any] = [
                objectIDKey as NSString: objectID,
                self.relatedObjectIDKey as NSString: relatedObjectID,
            ]
            _ = database.rd_insertRow(with: d, insertType: .orIgnore, tableName: self.name)
        }
    }

    // MARK: Fetching

    private func fetchRelatedObjectsWithIDs(
        _ relatedObjectIDs: Set<String>,
        _ database: FMDatabase
    )
        -> [DatabaseObject]?
    {
    ...
    private func fetchRelatedObjectIDsMap(_ objectIDs: Set<String>, _ database: FMDatabase) -> RelatedObjectIDsMap? {
    ...
    private func fetchLookupValues(_ objectIDs: Set<String>, _ database: FMDatabase) -> Set<LookupValue>? {
        guard
            !objectIDs.isEmpty, let resultSet = database.rd_selectRowsWhereKey(
                objectIDKey,
                inValues: Array(objectIDs),
                tableName: name
            ) else
        {
            return nil
        }
        return self.lookupValuesWithResultSet(resultSet)
    }

    private func lookupValuesWithResultSet(_ resultSet: FMResultSet) -> Set<LookupValue> {
        resultSet.mapToSet(self.lookupValueWithRow)
    }

    private func lookupValueWithRow(_ row: FMResultSet) -> LookupValue? {
    ...
    private func objectIDsSubtractingObjectIDsWithNoRelatedObjects(_ objectIDs: Set<String>) -> Set<String> {
        self.objectIDsWithNoRelatedObjects.withLock { objectIDs.subtracting($0) }
    }

    private func cacheObjectIDsWithNoRelatedObjects(_ objectIDs: Set<String>) {
        self.objectIDsWithNoRelatedObjects.withLock { $0.formUnion(objectIDs) }
    }

    private func uncacheObjectIDsWithNoRelatedObjects(_ objectIDs: Set<String>) {
        self.objectIDsWithNoRelatedObjects.withLock { $0.subtract(objectIDs) }
    }
}
