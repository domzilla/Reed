...
final class OPMLNormalizer {
    ...
    private func normalize(_ items: [RSOPMLItem], parentFolder: RSOPMLItem? = nil) {
        ...
        for item in items {
            ...
            guard let _ = item.titleFromAttributes else {
                ...
                if let itemChildren = item.children {
                    self.normalize(itemChildren, parentFolder: parentFolder)
                }
                continue
            }
            ...
            if let itemChildren = item.children {
                if let parentFolder {
                    ...
                    self.normalize(itemChildren, parentFolder: item)
                }
            }
        }
        ...
        if let parentFolder {
            for feed in feedsToAdd {
                if
                    !(parentFolder.children?
                        .contains(where: { $0.feedSpecifier?.feedURL == feed.feedSpecifier?.feedURL }) ?? false)
                {
            ...
            for feed in feedsToAdd {
                self.normalizedOPMLItems.append(feed)
            }
        }
    }
}
