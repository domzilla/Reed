...
final class CloudKitArticlesZone: CloudKitZone {
    ...
    func refreshArticles() async throws {
        do {
            ...
            if case CloudKitZoneError.userDeletedZone = error {
                ...
                try await self.refreshArticles()
            } else {
                throw error
            }
        }
    ...
    func modifyArticles(_ statusUpdates: [CloudKitArticleStatusUpdate]) async throws {
        ...
        for statusUpdate in statusUpdates {
            ...
            case .delete:
                deleteRecordIDs.append(CKRecord.ID(
                    recordName: self.statusID(statusUpdate.articleID),
                    zoneID: self.zoneID
                ))
            case .statusOnly:
                modifyRecords.append(self.makeStatusRecord(statusUpdate))
                deleteRecordIDs.append(CKRecord.ID(
                    recordName: self.articleID(statusUpdate.articleID),
                    zoneID: self.zoneID
                ))
            }
        }
...
extension CloudKitArticlesZone {
    private func handleModifyArticlesError(_ error: Error, statusUpdates: [CloudKitArticleStatusUpdate]) async throws {
        if case CloudKitZoneError.userDeletedZone = error {
            try await createZoneRecord()
            try await self.modifyArticles(statusUpdates)
        } else {
            throw error
        }
    }

    private func statusID(_ id: String) -> String {
        "s|\(id)"
    }

    private func articleID(_ id: String) -> String {
        "a|\(id)"
    }

    @MainActor
    private func makeStatusRecord(_ article: Article) -> CKRecord {
        let recordID = CKRecord.ID(recordName: self.statusID(article.articleID), zoneID: self.zoneID)
    ...
    @MainActor
    private func makeStatusRecord(_ statusUpdate: CloudKitArticleStatusUpdate) -> CKRecord {
        let recordID = CKRecord.ID(recordName: self.statusID(statusUpdate.articleID), zoneID: self.zoneID)
        let record = CKRecord(recordType: CloudKitArticleStatus.recordType, recordID: recordID)
    ...
    @MainActor
    private func makeArticleRecord(_ article: Article) -> CKRecord {
        let recordID = CKRecord.ID(recordName: self.articleID(article.articleID), zoneID: self.zoneID)
        let record = CKRecord(recordType: CloudKitArticle.recordType, recordID: recordID)

        let articleStatusRecordID = CKRecord.ID(recordName: self.statusID(article.articleID), zoneID: self.zoneID)
        record[CloudKitArticle.Fields.articleStatus] = CKRecord.Reference(
            recordID: articleStatusRecordID,
            action: .deleteSelf
        )
        ...
        if let authors = article.authors, !authors.isEmpty {
            for author in authors {
                ...
                if
                    let data = try? encoder.encode(parsedAuthor), let encodedParsedAuthor = String(
                        data: data,
                        encoding: .utf8
                    )
                {
    ...
    private nonisolated func compressArticleRecords(_ records: [CKRecord]) -> [CKRecord] {
        var result = [CKRecord]()
