...
final class Downloader {
    ...
    func download(_ url: URL) async throws -> (Data?, URLResponse?) {
        try await withCheckedThrowingContinuation { continuation in
            self.download(url) { data, response, error in
                if let error {
    ...
    func download(_ url: URL, _ callback: @escaping DownloadCallback) {
        ...
        self.download(URLRequest(url: url), callback)
    }

    func download(_ urlRequest: URLRequest, _ callback: @escaping DownloadCallback) {
        assert(Thread.isMainThread)
