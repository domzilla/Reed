...
public final class FeedFinder: Sendable {
    ...
    public static func find(url: URL) async throws -> Set<FeedSpecifier> {
        ...
        if FeedFinder.isFeed(data, url.absoluteString) {
            let feedSpecifier = FeedSpecifier(
                title: nil,
...
extension FeedFinder {
    fileprivate static func addFeedSpecifier(
        _ feedSpecifier: FeedSpecifier,
        feedSpecifiers: inout [String: FeedSpecifier]
    ) {
    ...
    fileprivate static func findFeedsInHTMLPage(htmlData: Data, urlString: String) async throws -> Set<FeedSpecifier> {
        ...
        let possibleFeedSpecifiers = self.possibleFeedsInHTMLPage(htmlData: htmlData, urlString: urlString)
        ...
        for oneFeedSpecifier in possibleFeedSpecifiers {
            if oneFeedSpecifier.source == .HTMLHead {
                self.addFeedSpecifier(oneFeedSpecifier, feedSpecifiers: &feedSpecifiers)
        ...
        if didFindFeedInHTMLHead {
            ...
            return await self.downloadFeedSpecifiers(feedSpecifiersToDownload, feedSpecifiers: feedSpecifiers)
        }
    }

    fileprivate static func possibleFeedsInHTMLPage(htmlData: Data, urlString: String) -> Set<FeedSpecifier> {
        ...
        if feedSpecifiers.isEmpty {
            ...
            if let url = URL(string: urlString) {
                ...
                let wordpressFeedSpecifier = FeedSpecifier(
                    title: nil,
                ...
                let indexXMLFeedSpecifier = FeedSpecifier(
                    title: nil,
                    urlString: indexXMLURL.absoluteString,
                    source: .HTMLLink,
                    orderFound: 1
                )
    ...
    fileprivate nonisolated static func isHTML(_ data: Data) -> Bool {
        data.isProbablyHTML
    }

    fileprivate static func downloadFeedSpecifiers(
        _ downloadFeedSpecifiers: Set<FeedSpecifier>,
        feedSpecifiers: [String: FeedSpecifier]
    ) async
        -> Set<FeedSpecifier>
    {
        ...
        await withTaskGroup(of: FeedSpecifier?.self) { group in
            ...
            for await feedSpecifier in group {
                if let feedSpecifier {
                    self.addFeedSpecifier(feedSpecifier, feedSpecifiers: &resultFeedSpecifiers)
                }
            }
        }
    ...
    fileprivate nonisolated static func isFeed(_ data: Data, _ urlString: String) -> Bool {
