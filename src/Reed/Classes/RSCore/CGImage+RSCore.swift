...
extension CGImage {
    func calculateLuminanceType() -> ImageLuminanceType? {
        ...
        // 8 bits for each color channel, we're doing ARGB so 32 bits (4 bytes) total, and thus if the image is n pixels
        // wide,
        // and has 4 bytes per pixel, the total bytes per row is 4n. That gives us 2^8 = 256 color variations for each
        // RGB channel
        // or 256 * 256 * 256 = ~16.7M color options in total. That seems like a lot, but lots of HDR movies are in 10
        // bit, which
        ...
        for i in 0..<totalPixels {
            ...
            let r = self.red(for: pixel)
            let g = self.green(for: pixel)
            let b = self.blue(for: pixel)
