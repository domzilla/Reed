...
final class AppConfig {
    static let appName: String = (Bundle.main.infoDictionary!["CFBundleExecutable"]! as! String)

    static let cacheFolder: URL = {
        let folderURL: URL

        if
            let userCacheFolder = try? FileManager.default.url(
                for: .cachesDirectory,
                in: .userDomainMask,
                appropriateFor: nil,
                create: true
    ...
    static func cacheSubfolder(named name: String) -> URL {
        self.ensureSubfolder(named: name, folderURL: self.cacheFolder)
    }
    ...
    static func dataSubfolder(named name: String) -> URL {
        self.ensureSubfolder(named: name, folderURL: self.dataFolder)
    }

    static func ensureSubfolder(named name: String, folderURL: URL) -> URL {
...
extension AppConfig {
    fileprivate static func createFolderIfNecessary(_ folderURL: URL) {
