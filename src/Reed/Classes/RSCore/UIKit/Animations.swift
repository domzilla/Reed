//
//  Animations.swift
//  RSCore
//
//  Created by Maurice Parker on 1/27/20.
//  Copyright © 2020 Ranchero Software. All rights reserved.
//

import Foundation

/// Used to select which animations should be performed
struct Animations: OptionSet, Sendable {
    /// Selections and deselections will be animated.
    static let select = Animations(rawValue: 1)

    /// Scrolling will be animated
    static let scroll = Animations(rawValue: 2)

    /// Pushing and popping navigation view controllers will be animated
    static let navigation = Animations(rawValue: 4)

    let rawValue: Int
    init(rawValue: Int) {
        self.rawValue = rawValue
    }
}
