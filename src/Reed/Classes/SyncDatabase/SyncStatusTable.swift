...
extension SyncStatusTable {
    fileprivate nonisolated static func statusWithRow(_ row: FMResultSet) -> SyncStatus? {
        guard
            let articleID = row.string(forColumn: SyncDatabaseKey.articleID),
        ...
        let flag = row.bool(forColumn: SyncDatabaseKey.flag)
        let selected = row.bool(forColumn: SyncDatabaseKey.selected)

        return SyncStatus(articleID: articleID, key: key, flag: flag, selected: selected)
    }

    fileprivate nonisolated static func selectPendingArticleIDs(
