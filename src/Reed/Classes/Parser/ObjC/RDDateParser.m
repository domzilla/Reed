...
} RDTimeZoneAbbreviationAndOffset;


#define kNumberOfTimeZones 96

static const RDTimeZoneAbbreviationAndOffset timeZoneTable[kNumberOfTimeZones] = {
...
typedef NS_ENUM(NSInteger, RDMonth) {
	RDMonthJanuary = 1,
	RDMonthFebruary,
	RDMonthMarch,
	RDMonthApril,
...
static NSInteger nextMonthValue(const char *bytes, NSUInteger numberOfBytes, NSUInteger startingIndex, NSUInteger *finalIndex) {
		...
		*finalIndex = i;
		...
		if (numberOfAlphaCharactersFound == 1) {
			if (character == 'F' || character == 'f')
				return RDMonthFebruary;
			if (character == 'S' || character == 's')
				return RDMonthSeptember;
			if (character == 'O' || character == 'o')
				return RDMonthOctober;
			if (character == 'N' || character == 'n')
				return RDMonthNovember;
			if (character == 'D' || character == 'd')
				return RDMonthDecember;
		}
	...
	if (monthCharacters[0] == 'A' || monthCharacters[0] == 'a') { //April, August
		if (monthCharacters[1] == 'u' || monthCharacters[1] == 'U')
			return RDMonthAugust;
		return RDMonthApril;
	}

	return RDMonthJanuary; //should never get here
}
...
static NSInteger offsetInSecondsForTimeZoneAbbreviation(const char *abbreviation) {
	...
	for (i = 0; i < kNumberOfTimeZones; i++) {
		...
		RDTimeZoneAbbreviationAndOffset zone = timeZoneTable[i];
...
static const char *rd_GMT = "GMT";
static const char *rd_UTC = "UTC";
...
static NSInteger parsedTimeZoneOffset(const char *bytes, NSUInteger numberOfBytes, NSUInteger startingIndex) {
	...
	if (numberOfCharactersFound < 1 || timeZoneCharacters[0] == 'Z' || timeZoneCharacters[0] == 'z')
		return 0;
	if (strcasestr(timeZoneCharacters, rd_GMT) != nil || strcasestr(timeZoneCharacters, rd_UTC))
		return 0;
...
static NSDate *dateWithYearMonthDayHourMinuteSecondAndTimeZoneOffset(NSInteger year, NSInteger month, NSInteger day, NSInteger hour, NSInteger minute, NSInteger second, NSInteger milliseconds, NSInteger timeZoneOffset) {
	...
	if (rawTime == (time_t)ULONG_MAX) {
		...
		/*NSCalendar is super-amazingly-slow (which is partly why RDDateParser exists), so this is used only when the date is far enough in the future (19 January 2038 03:14:08Z on 32-bit systems) that timegm fails. If profiling says that this is a performance issue, then you've got a weird app that needs to work with dates far in the future.*/
...
static NSDate *RDParsePubDateWithBytes(const char *bytes, NSUInteger numberOfBytes) {
	...
	NSUInteger finalIndex = 0;
	NSInteger day = 1;
	NSInteger month = RDMonthJanuary;
	NSInteger year = 1970;
	NSInteger hour = 0;
	NSInteger minute = 0;
	NSInteger second = 0;
...
static NSDate *RDParseW3CWithBytes(const char *bytes, NSUInteger numberOfBytes) {
...
NSDate *RDDateWithBytes(const char *bytes, NSUInteger numberOfBytes) {

	if (numberOfBytesIsOutsideReasonableRange(numberOfBytes))
		return nil;

	if (dateIsW3CDate(bytes, numberOfBytes)) {
		return RDParseW3CWithBytes(bytes, numberOfBytes);
	}
	if (dateIsPubDate(bytes, numberOfBytes))
		return RDParsePubDateWithBytes(bytes, numberOfBytes);
...
NSDate *RDDateWithString(NSString *dateString) {

	const char *utf8String = [dateString UTF8String];
	return RDDateWithBytes(utf8String, strlen(utf8String));
}
