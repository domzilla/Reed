...
@interface RDOPMLParser () <RDSAXParserDelegate>

@property (nonatomic, readwrite) RDOPMLDocument *OPMLDocument;
...
void RDParseOPML(ParserData *parserData, OPMLParserCallback callback) {
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		...
		@autoreleasepool {
			NSError *error = nil;
			RDOPMLDocument *opmlDocument = [RDOPMLParser parseOPMLWithParserData:parserData error:&error];
...
@implementation RDOPMLParser

#pragma mark - Class Methods

+ (RDOPMLDocument *)parseOPMLWithParserData:(ParserData *)parserData error:(NSError **)error {

	RDOPMLParser *parser = [[RDOPMLParser alloc] initWithParserData:parserData];

	RDOPMLDocument *document = parser.OPMLDocument;
...
- (void)parse:(ParserData *)parserData {
	...
	@autoreleasepool {
		...
		if (![self canParseData:parserData.data]) {
			...
			self.error = RDOPMLWrongFormatError(filename);
			return;
		}
		
		RDSAXParser *parser = [[RDSAXParser alloc] initWithDelegate:self];

		self.itemStack = [NSMutableArray new];
		self.OPMLDocument = [RDOPMLDocument new];
...
- (void)saxParser:(RDSAXParser *)SAXParser XMLStartElement:(const xmlChar *)localName prefix:(const xmlChar *)prefix uri:(const xmlChar *)uri numberOfNamespaces:(NSInteger)numberOfNamespaces namespaces:(const xmlChar **)namespaces numberOfAttributes:(NSInteger)numberOfAttributes numberDefaulted:(int)numberDefaulted attributes:(const xmlChar **)attributes {
	...
	if (RDSAXEqualTags(localName, kTitle, kTitleLength)) {
		[SAXParser beginStoringCharacters];
		return;
	}

	if (!RDSAXEqualTags(localName, kOutline, kOutlineLength)) {
		return;
	}

	RDOPMLItem *item = [RDOPMLItem new];
...
- (void)saxParser:(RDSAXParser *)SAXParser XMLEndElement:(const xmlChar *)localName prefix:(const xmlChar *)prefix uri:(const xmlChar *)uri {
	...
	if (RDSAXEqualTags(localName, kTitle, kTitleLength)) {
		RDOPMLItem* item = [self currentItem];
		if ([item isKindOfClass:[RDOPMLDocument class]]) {
			((RDOPMLDocument *)item).title = SAXParser.currentStringWithTrimmedWhitespace;
		}
		return;
	}
...
- (NSString *)saxParser:(RDSAXParser *)SAXParser internedStringForName:(const xmlChar *)name prefix:(const xmlChar *)prefix {
	...
	if (nameLength == kTextLength - 1) {
		if (RDSAXEqualTags(name, kText, kTextLength)) {
			return OPMLTextKey;
		}
		...
		if (RDSAXEqualTags(name, kXMLURL, kXMLURLLength)) {
			return OPMLXMLURLKey;
		}
	}

	else if (nameLength == kVersionLength - 1) {
		if (RDSAXEqualTags(name, kVersion, kVersionLength)) {
			return OPMLVersionKey;
		}
		if (RDSAXEqualTags(name, kHTMLURL, kHTMLURLLength)) {
			return OPMLHMTLURLKey;
		}
	}

	else if (nameLength == kDescriptionLength - 1) {
		if (RDSAXEqualTags(name, kDescription, kDescriptionLength)) {
