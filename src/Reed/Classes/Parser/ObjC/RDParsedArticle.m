...
@implementation RDParsedArticle
...
- (void)addEnclosure:(RDParsedEnclosure *)enclosure {
...
- (void)addAuthor:(RDParsedAuthor *)author {
...
- (NSString *)calculatedArticleID {
	...
	if (!RDParserStringIsEmpty(self.permalink) && datePublishedTimeStampString) {
		[s appendString:self.permalink];
		[s appendString:datePublishedTimeStampString];
	}

	else if (!RDParserStringIsEmpty(self.link) && datePublishedTimeStampString) {
		[s appendString:self.link];
		[s appendString:datePublishedTimeStampString];
	}

	else if (!RDParserStringIsEmpty(self.title) && datePublishedTimeStampString) {
	...
	else if (!RDParserStringIsEmpty(self.permalink)) {
		[s appendString:self.permalink];
	}

	else if (!RDParserStringIsEmpty(self.link)) {
		[s appendString:self.link];
	}

	else if (!RDParserStringIsEmpty(self.title)) {
		[s appendString:self.title];
	}
	
	else if (!RDParserStringIsEmpty(self.body)) {
		[s appendString:self.body];
	}

	return [s rdparser_md5Hash];
}
