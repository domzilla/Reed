...
- (NSString *)rdparser_stringByDecodingHTMLEntities {
	...
	@autoreleasepool {
		...
		while (true) {
			...
			if ([scanner rd_scanEntityValue:&decodedEntity]) {
...
static NSDictionary *RDEntitiesDictionary(void);
static NSString *RDParserStringWithValue(uint32_t value);

- (NSString * _Nullable)rd_stringByDecodingEntity {
	...
	NSDictionary *entitiesDictionary = RDEntitiesDictionary();
	...
	if ([s hasPrefix:@"#x"] || [s hasPrefix:@"#X"]) { // Hex
		...
		if ([scanner scanHexInt:&hexValue]) {
			return RDParserStringWithValue((uint32_t)hexValue);
		}
		return nil;
	}
		...
		return RDParserStringWithValue((uint32_t)value);
	}

	return nil;
}

- (NSString *)rdparser_stringByEncodingRequiredEntities {
...
- (NSString *)rdparser_md5Hash {
	...
	NSData *md5Data = [self _rdparser_md5HashData];
...
- (BOOL)rd_scanEntityValue:(NSString * _Nullable * _Nullable)decodedEntity {
	...
	dispatch_once(&onceToken, ^{
		for (NSString *entityName in RDEntitiesDictionary().allKeys) {
	...
	while (true) {
		...
		if (ch == ';') {
			...
			*decodedEntity = [rawEntity rd_stringByDecodingEntity];
...
static NSDictionary *RDEntitiesDictionary(void) {
	...
	dispatch_once(&onceToken, ^{
		...
		entitiesDictionary = @{
			...
			@"sect": @"§",
			@"smallcircle": @"◦",
			@"shy": RDParserStringWithValue(173),
			@"sup1": @"¹",
			@"sup2": @"²",
			@"sup3": @"³",
			@"szlig": @"ß",
			...
			@"yuml": @"ÿ",
			@"infin": @"∞",
			@"nbsp": RDParserStringWithValue(160)
		};
	});
