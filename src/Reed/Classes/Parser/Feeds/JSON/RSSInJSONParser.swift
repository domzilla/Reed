...
extension RSSInJSONParser {
    fileprivate static func parseItems(_ itemsObject: JSONArray, _ feedURL: String) -> Set<ParsedItem> {
        Set(itemsObject.compactMap { oneItemDictionary -> ParsedItem? in
            return self.parsedItemWithDictionary(oneItemDictionary, feedURL)
        })
    }

    fileprivate static func parsedItemWithDictionary(
        _ itemDictionary: JSONDictionary,
        _ feedURL: String
    )
        -> ParsedItem?
    {
        ...
        if let datePublishedString = itemDictionary["pubDate"] as? String {
            datePublished = RDDateWithString(datePublishedString)
        }

        let authors = self.parseAuthors(itemDictionary)
        let tags = self.parseTags(itemDictionary)
        ...
        if uniqueID == nil {
            ...
            uniqueID = (s as NSString).rdparser_md5Hash()
        }
    ...
    fileprivate static func parseAuthors(_ itemDictionary: JSONDictionary) -> Set<ParsedAuthor>? {
    ...
    fileprivate static func parseTags(_ itemDictionary: JSONDictionary) -> Set<String>? {
