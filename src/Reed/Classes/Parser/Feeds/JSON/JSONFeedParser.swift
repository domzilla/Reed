...
enum JSONFeedParser {
    ...
    static let jsonFeedVersionMarker =
    ...
    static func parse(_ parserData: ParserData) throws -> ParsedFeed? {
        ...
        guard
            let version = d[Key.version] as? String,
            let _ = version.range(of: JSONFeedParser.jsonFeedVersionMarker) else
        {
...
extension JSONFeedParser {
    fileprivate static func parseAuthors(_ dictionary: JSONDictionary) -> Set<ParsedAuthor>? {
        ...
        guard
            let authorDictionary = dictionary[Key.author] as? JSONDictionary,
    ...
    fileprivate static func parseAuthor(_ dictionary: JSONDictionary) -> ParsedAuthor? {
    ...
    fileprivate static func parseHubs(_ dictionary: JSONDictionary) -> Set<ParsedHub>? {
        ...
        let hubs = hubsArray.compactMap { hubDictionary -> ParsedHub? in
            guard
                let hubURL = hubDictionary[Key.url] as? String,
                let hubType = hubDictionary[Key.type] as? String else
            {
                return nil
            }
    ...
    fileprivate static func parseItems(_ itemsArray: JSONArray, _ feedURL: String) -> Set<ParsedItem> {
        Set(itemsArray.compactMap { oneItemDictionary -> ParsedItem? in
            return self.parseItem(oneItemDictionary, feedURL)
        })
    }

    fileprivate static func parseItem(_ itemDictionary: JSONDictionary, _ feedURL: String) -> ParsedItem? {
        ...
        let title = self.parseTitle(itemDictionary, feedURL)
        ...
        let datePublished = self.parseDate(itemDictionary[Key.datePublished] as? String)
        let dateModified = self.parseDate(itemDictionary[Key.dateModified] as? String)

        let authors = self.parseAuthors(itemDictionary)
        var tags: Set<String>? = nil
        if let tagsArray = itemDictionary[Key.tags] as? [String] {
            tags = Set(tagsArray)
        }
        let attachments = self.parseAttachments(itemDictionary)
    ...
    fileprivate static func parseTitle(_ itemDictionary: JSONDictionary, _ feedURL: String) -> String? {
        guard let title = itemDictionary[Key.title] as? String else {
            return nil
        }

        if self.isSpecialCaseTitleWithEntitiesFeed(feedURL) {
            return (title as NSString).rdparser_stringByDecodingHTMLEntities()
        }

        return title
    }

    fileprivate static func isSpecialCaseTitleWithEntitiesFeed(_ feedURL: String) -> Bool {
    ...
    fileprivate static func parseUniqueID(_ itemDictionary: JSONDictionary) -> String? {
        if let uniqueID = itemDictionary[Key.uniqueID] as? String {
            return uniqueID // Spec says it must be a string
        }
        // Version 1 spec also says that if it’s a number, even though that’s incorrect, it should be coerced to a
        // string.
    ...
    fileprivate static func parseDate(_ dateString: String?) -> Date? {
        guard let dateString, !dateString.isEmpty else {
            return nil
        }
        return RDDateWithString(dateString)
    }

    fileprivate static func parseAttachments(_ itemDictionary: JSONDictionary) -> Set<ParsedAttachment>? {
        guard let attachmentsArray = itemDictionary[Key.attachments] as? JSONArray else {
            return nil
        }
        return Set(attachmentsArray.compactMap { self.parseAttachment($0) })
    }

    fileprivate static func parseAttachment(_ attachmentObject: JSONDictionary) -> ParsedAttachment? {
