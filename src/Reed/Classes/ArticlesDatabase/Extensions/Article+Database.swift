...
extension Article {
    ...
    convenience init(
        ...
        self.init(
            accountID: accountID,
            articleID: parsedItem.syncServiceID,
            feedID: feedID,
            uniqueID: parsedItem.uniqueID,
            title: parsedItem.title,
            contentHTML: parsedItem.contentHTML,
            contentText: parsedItem.contentText,
            markdown: parsedItem.markdown,
            url: parsedItem.url,
            externalURL: parsedItem.externalURL,
    ...
    private func addPossibleStringChangeWithKeyPath(
        _ comparisonKeyPath: KeyPath<Article, String?>,
        _ otherArticle: Article,
        _ key: String,
        _ dictionary: inout DatabaseDictionary
    ) {
    ...
    func byAdding(_ authors: Set<Author>) -> Article {
        ...
        return Article(
            ...
            url: self.rawLink,
            externalURL: self.rawExternalLink,
            summary: self.summary,
            imageURL: self.rawImageLink,
    ...
    func changesFrom(_ existingArticle: Article) -> DatabaseDictionary? {
        ...
        self.addPossibleStringChangeWithKeyPath(\Article.title, existingArticle, DatabaseKey.title, &d)
        self.addPossibleStringChangeWithKeyPath(\Article.contentHTML, existingArticle, DatabaseKey.contentHTML, &d)
        self.addPossibleStringChangeWithKeyPath(\Article.contentText, existingArticle, DatabaseKey.contentText, &d)
        self.addPossibleStringChangeWithKeyPath(\Article.rawLink, existingArticle, DatabaseKey.url, &d)
        self.addPossibleStringChangeWithKeyPath(\Article.rawExternalLink, existingArticle, DatabaseKey.externalURL, &d)
        self.addPossibleStringChangeWithKeyPath(\Article.summary, existingArticle, DatabaseKey.summary, &d)
        self.addPossibleStringChangeWithKeyPath(\Article.rawImageLink, existingArticle, DatabaseKey.imageURL, &d)
    ...
    //	static func articlesWithParsedItems(_ parsedItems: Set<ParsedItem>, _ accountID: String, _ feedID: String, _
    //	statusesDictionary: [String: ArticleStatus]) -> Set<Article> {
    //		let maximumDateAllowed = Date().addingTimeInterval(60 * 60 * 24) // Allow dates up to about 24 hours ahead
    //		of now
    //		return Set(parsedItems.map{ Article(parsedItem: $0, maximumDateAllowed: maximumDateAllowed, accountID:
    //		accountID, feedID: feedID, status: statusesDictionary[$0.articleID]!) })
    //	}

    private static func _maximumDateAllowed() -> Date {
        Date().addingTimeInterval(60 * 60 * 24) // Allow dates up to about 24 hours ahead of now
    }

    static func articlesWithFeedIDsAndItems(
        _ feedIDsAndItems: [String: Set<ParsedItem>],
        _ accountID: String,
        _ statusesDictionary: [String: ArticleStatus]
    )
        -> Set<Article>
    {
        let maximumDateAllowed = self._maximumDateAllowed()
        ...
        for (feedID, parsedItems) in feedIDsAndItems {
            for parsedItem in parsedItems {
                ...
                let article = Article(
                    parsedItem: parsedItem,
                    maximumDateAllowed: maximumDateAllowed,
                    accountID: accountID,
                    feedID: feedID,
                    status: status
                )
    ...
    {
        let maximumDateAllowed = self._maximumDateAllowed()
        return Set(parsedItems.map { Article(
            parsedItem: $0,
            maximumDateAllowed: maximumDateAllowed,
            accountID: accountID,
            feedID: feedID,
            status: statusesDictionary[$0.articleID]!
        ) })
    }
}
...
extension Article: DatabaseObject {
    ...
    private nonisolated func databaseObjectArray(with objects: Set<some DatabaseObject>?) -> [DatabaseObject]? {
...
extension Set<Article> {
    ...
    func databaseObjects() -> [DatabaseObject] {
        self.map { $0 as DatabaseObject }
    }

    func databaseDictionaries() -> [DatabaseDictionary]? {
        self.compactMap { $0.databaseDictionary() }
    }
}
