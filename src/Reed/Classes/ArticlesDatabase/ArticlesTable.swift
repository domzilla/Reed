...
final class ArticlesTable: DatabaseTable, Sendable {
    ...
    init(name: String, accountID: String, queue: DatabaseQueue, retentionStyle: ArticlesDatabase.RetentionStyle) {
        ...
        self.authorsLookupTable = DatabaseLookupTable(
            name: DatabaseTableName.authorsLookup,
            objectIDKey: DatabaseKey.articleID,
            relatedObjectIDKey: DatabaseKey.authorID,
            relatedTable: authorsTable,
            relationshipName: RelationshipName.authors
        )
    ...
    func fetchArticles(_ feedID: String) throws -> Set<Article> {
        try self.fetchArticles { self.fetchArticlesForFeedID(feedID, $0) }
    }

    func fetchArticlesAsync(_ feedID: String, _ completion: @escaping ArticleSetResultBlock) {
        self.fetchArticlesAsync({ self.fetchArticlesForFeedID(feedID, $0) }, completion)
    }

    func fetchArticles(_ feedIDs: Set<String>) throws -> Set<Article> {
        try self.fetchArticles { self.fetchArticles(feedIDs, $0) }
    }

    func fetchArticlesAsync(_ feedIDs: Set<String>, _ completion: @escaping ArticleSetResultBlock) {
        self.fetchArticlesAsync({ self.fetchArticles(feedIDs, $0) }, completion)
    }

    // MARK: - Fetching Articles by articleID

    func fetchArticles(articleIDs: Set<String>) throws -> Set<Article> {
        try self.fetchArticles { self.fetchArticles(articleIDs: articleIDs, $0) }
    }

    func fetchArticlesAsync(articleIDs: Set<String>, _ completion: @escaping ArticleSetResultBlock) {
        self.fetchArticlesAsync({ self.fetchArticles(articleIDs: articleIDs, $0) }, completion)
    }

    // MARK: - Fetching Unread Articles

    func fetchUnreadArticles(_ feedIDs: Set<String>, _ limit: Int?) throws -> Set<Article> {
        try self.fetchArticles { self.fetchUnreadArticles(feedIDs, limit, $0) }
    }

    func fetchUnreadArticlesAsync(
        _ feedIDs: Set<String>,
        _ limit: Int?,
        _ completion: @escaping ArticleSetResultBlock
    ) {
        self.fetchArticlesAsync({ self.fetchUnreadArticles(feedIDs, limit, $0) }, completion)
    }
    ...
    func fetchArticlesSince(_ feedIDs: Set<String>, _ cutoffDate: Date, _ limit: Int?) throws -> Set<Article> {
        try self.fetchArticles { self.fetchArticlesSince(feedIDs, cutoffDate, limit, $0) }
    }

    func fetchArticlesSinceAsync(
        _ feedIDs: Set<String>,
        _ cutoffDate: Date,
        _ limit: Int?,
        _ completion: @escaping ArticleSetResultBlock
    ) {
        self.fetchArticlesAsync({ self.fetchArticlesSince(feedIDs, cutoffDate, limit, $0) }, completion)
    }

    // MARK: - Fetching Starred Articles

    func fetchStarredArticles(_ feedIDs: Set<String>, _ limit: Int?) throws -> Set<Article> {
        try self.fetchArticles { self.fetchStarredArticles(feedIDs, limit, $0) }
    }

    func fetchStarredArticlesAsync(
        _ feedIDs: Set<String>,
        _ limit: Int?,
        _ completion: @escaping ArticleSetResultBlock
    ) {
        self.fetchArticlesAsync({ self.fetchStarredArticles(feedIDs, limit, $0) }, completion)
    }

    func fetchStarredArticlesCount(_ feedIDs: Set<String>) throws -> Int {
        try fetchArticlesCount { self.fetchStarredArticlesCount(feedIDs, $0) }
    }
    ...
    func fetchArticlesMatchingAsync(
        _ searchString: String,
        _ feedIDs: Set<String>,
        _ completion: @escaping ArticleSetResultBlock
    ) {
        self.fetchArticlesAsync({ self.fetchArticlesMatching(searchString, feedIDs, $0) }, completion)
    }
    ...
    func fetchArticlesMatchingWithArticleIDsAsync(
        ...
        self.fetchArticlesAsync({ self.fetchArticlesMatchingWithArticleIDs(searchString, articleIDs, $0) }, completion)
    }
    ...
    func update(
        ...
        self.queue.runInTransaction { databaseResult in
            func makeDatabaseCalls(_ database: FMDatabase) {
                ...
                let (statusesDictionary, _) = self.statusesTable.ensureStatusesForArticleIDs(
                    articleIDs,
                    false,
                    database
                ) // 1
                ...
                let incomingArticles = Article.articlesWithParsedItems(
                    parsedItems,
                    feedID,
                    self.accountID,
                    statusesDictionary
                ...
                let newArticles = self
                    .findAndSaveNewArticles(incomingArticles, fetchedArticlesDictionary, database) // 5
                let updatedArticles = self.findAndSaveUpdatedArticles(
                    incomingArticles,
                ...
                if deleteOlder {
                    ...
                    articlesToDelete = fetchedArticles.filter { article -> Bool in
                        return !article.status.starred && article.status.dateArrived < cutoffDate && !articleIDs
    ...
    func update(
        _ feedIDsAndItems: [String: Set<ParsedItem>],
        _ read: Bool,
        _ completion: @escaping UpdateArticlesCompletionBlock
    ) {
        ...
        self.queue.runInTransaction { databaseResult in
            func makeDatabaseCalls(_ database: FMDatabase) {
                ...
                let allIncomingArticles = Article.articlesWithFeedIDsAndItems(
                    feedIDsAndItems,
                    self.accountID,
                    statusesDictionary
                ) // 2
    ...
    func delete(articleIDs: Set<String>, completion: DatabaseCompletionBlock?) {
        self.queue.runInTransaction { databaseResult in
            func makeDatabaseCalls(_ database: FMDatabase) {
    ...
    func fetchUnreadCount(
        _ feedIDs: Set<String>,
        _ since: Date,
        _ completion: @escaping SingleUnreadCountCompletionBlock
    ) {
    ...
    func fetchUnreadArticleIDsAsync(_ completion: @escaping ArticleIDsCompletionBlock) {
        self.statusesTable.fetchArticleIDsAsync(.read, false, completion)
    }

    func fetchStarredArticleIDsAsync(_ completion: @escaping ArticleIDsCompletionBlock) {
        self.statusesTable.fetchArticleIDsAsync(.starred, true, completion)
    }

    func fetchStarredArticleIDs() throws -> Set<String> {
        try self.statusesTable.fetchStarredArticleIDs()
    }

    func fetchArticleIDsForStatusesWithoutArticlesNewerThanCutoffDate(
        _ completion: @escaping ArticleIDsCompletionBlock
    ) {
        self.statusesTable.fetchArticleIDsForStatusesWithoutArticlesNewerThan(self.articleCutoffDate, completion)
    }
    ...
    func mark(
        ...
        _ flag: Bool,
        _ completion: @escaping ArticleStatusesResultBlock
    ) {
        self.queue.runInTransaction { databaseResult in
            switch databaseResult {
            case let .success(database):
...
nonisolated extension ArticlesTable {
    // MARK: - Fetching
    ...
    private func fetchArticlesAsync(
        _ fetchMethod: @escaping ArticlesFetchMethod,
        _ completion: @escaping ArticleSetResultBlock
    ) {
        self.queue.runInDatabase { databaseResult in
            switch databaseResult {
    ...
    private func articlesWithResultSet(_ resultSet: FMResultSet, _ database: FMDatabase) -> Set<Article> {
        ...
        let authorsMap = self.authorsLookupTable.fetchRelatedObjects(for: fetchedArticleIDs, in: database)
        ...
        self.articlesCache.withLock { articlesCache in
    ...
    private func fetchArticlesWithWhereClause(
        ...
        -> Set<Article>
    {
        ...
        return self.articlesWithSQL(sql, parameters, database)
    }

    private func fetchArticleCountsWithWhereClause(
        _ database: FMDatabase,
        whereClause: String,
        parameters: [AnyObject]
    )
    ...
    private func fetchArticlesMatching(_ searchString: String, _ database: FMDatabase) -> Set<Article> {
        let sql = "select rowid from search where search match ?;"
        let sqlSearchString = self.sqliteSearchString(with: searchString)
    ...
    private func sqliteSearchString(with searchString: String) -> String {
        var s = ""
        searchString
            .enumerateSubstrings(
                in: searchString.startIndex..<searchString.endIndex,
                options: .byWords
            ) { word, _, _, _ in
                guard let word else {
                    return
    ...
    private func articlesWithSQL(_ sql: String, _ parameters: [AnyObject], _ database: FMDatabase) -> Set<Article> {
        guard let resultSet = database.executeQuery(sql, withArgumentsIn: parameters) else {
            return Set<Article>()
        }
        return self.articlesWithResultSet(resultSet, database)
    }

    private func fetchArticles(_ feedIDs: Set<String>, _ database: FMDatabase) -> Set<Article> {
    ...
    private func fetchUnreadArticles(_ feedIDs: Set<String>, _ limit: Int?, _ database: FMDatabase) -> Set<Article> {
        // select * from articles natural join statuses where feedID in ('http://ranchero.com/xml/rss.xml') and read=0
        if feedIDs.isEmpty {
            return Set<Article>()
        }
    ...
    private func fetchArticlesForFeedID(_ feedID: String, _ database: FMDatabase) -> Set<Article> {
        self.fetchArticlesWithWhereClause(
            database,
            whereClause: "articles.feedID = ?",
            parameters: [feedID as AnyObject]
        )
    }
    ...
    private func fetchArticlesSince(
        _ feedIDs: Set<String>,
        _ cutoffDate: Date,
        _ limit: Int?,
        _ database: FMDatabase
        ...
        -> Set<Article>
    {
        // select * from articles natural join statuses where feedID in ('http://ranchero.com/xml/rss.xml') and
        // (datePublished > ? || (datePublished is null and dateArrived > ?)
        //
    ...
    private func fetchStarredArticles(_ feedIDs: Set<String>, _ limit: Int?, _ database: FMDatabase) -> Set<Article> {
        // select * from articles natural join statuses where feedID in ('http://ranchero.com/xml/rss.xml') and
        // starred=1;
        if feedIDs.isEmpty {
            return Set<Article>()
    ...
    private func fetchStarredArticlesCount(_ feedIDs: Set<String>, _ database: FMDatabase) -> Int {
        // select count from articles natural join statuses where feedID in ('http://ranchero.com/xml/rss.xml') and
        // starred=1;
        if feedIDs.isEmpty {
            return 0
        }
    ...
    private func fetchArticlesMatching(
        _ searchString: String,
        _ feedIDs: Set<String>,
        _ database: FMDatabase
    )
    ...
    private func fetchArticlesMatchingWithArticleIDs(
        _ searchString: String,
        _ articleIDs: Set<String>,
        _ database: FMDatabase
    )
    ...
    private func callUpdateArticlesCompletionBlock(
        _ newArticles: Set<Article>?,
        _ updatedArticles: Set<Article>?,
        _ deletedArticles: Set<Article>?,
        _ completion: @escaping UpdateArticlesCompletionBlock
    ) {
    ...
    private func findNewArticles(
        _ incomingArticles: Set<Article>,
        _ fetchedArticlesDictionary: [String: Article]
    )
        -> Set<Article>?
    {
    ...
    private func findAndSaveNewArticles(
        _ incomingArticles: Set<Article>,
        _ fetchedArticlesDictionary: [String: Article],
        _ database: FMDatabase
    )
    ...
    private func saveNewArticles(_ articles: Set<Article>, _ database: FMDatabase) {
        self.saveRelatedObjectsForNewArticles(articles, database)
    ...
    private func saveRelatedObjectsForNewArticles(_ articles: Set<Article>, _ database: FMDatabase) {
        let databaseObjects = articles.databaseObjects()
        self.authorsLookupTable.saveRelatedObjects(for: databaseObjects, in: database)
    }

    // MARK: - Updating Existing Articles

    private func articlesWithRelatedObjectChanges(
        _ comparisonKeyPath: KeyPath<Article, Set<some Any>?>,
        _ updatedArticles: Set<Article>,
        _ fetchedArticles: [String: Article]
    )
        -> Set<Article>
    {
        updatedArticles.filter { updatedArticle -> Bool in
    ...
    private func updateRelatedObjects(
        _ comparisonKeyPath: KeyPath<Article, Set<some Any>?>,
        _ updatedArticles: Set<Article>,
        _ fetchedArticles: [String: Article],
        _ lookupTable: DatabaseLookupTable,
        _ database: FMDatabase
    ) {
        let articlesWithChanges = self.articlesWithRelatedObjectChanges(
            comparisonKeyPath,
            updatedArticles,
            fetchedArticles
        )
    ...
    private func saveUpdatedRelatedObjects(
        _ updatedArticles: Set<Article>,
        _ fetchedArticles: [String: Article],
        _ database: FMDatabase
    ) {
        self.updateRelatedObjects(\Article.authors, updatedArticles, fetchedArticles, self.authorsLookupTable, database)
    }

    private func findUpdatedArticles(
        _ incomingArticles: Set<Article>,
        _ fetchedArticlesDictionary: [String: Article]
    )
        -> Set<Article>?
    ...
    private func findAndSaveUpdatedArticles(
        _ incomingArticles: Set<Article>,
        _ fetchedArticlesDictionary: [String: Article],
        _ database: FMDatabase
    )
    ...
    private func saveUpdatedArticles(
        _ updatedArticles: Set<Article>,
        _ fetchedArticles: [String: Article],
        _ database: FMDatabase
    ) {
        self.saveUpdatedRelatedObjects(updatedArticles, fetchedArticles, database)

        for updatedArticle in updatedArticles {
            self.saveUpdatedArticle(updatedArticle, fetchedArticles, database)
        }
    }

    private func saveUpdatedArticle(
        _ updatedArticle: Article,
        _ fetchedArticles: [String: Article],
        _ database: FMDatabase
    ) {
        ...
        guard let fetchedArticle = fetchedArticles[updatedArticle.articleID] else {
            assertionFailure("Expected to find matching fetched article.")
            self.saveNewArticles(Set([updatedArticle]), database)
            return
        }
        ...
        updateRowsWithDictionary(
            ...
            matches: updatedArticle.articleID,
            database: database
        )
    }

    private func addArticlesToCache(_ articles: Set<Article>?) {
    ...
    private func removeArticleIDsFromCache(_ articleIDs: Set<String>) {
        self.articlesCache.withLock { articlesCache in
    ...
    private func articleIsIgnorable(_ article: Article) -> Bool {
        if article.status.starred || !article.status.read {
            return false
        }
        return article.status.dateArrived < self.articleCutoffDate
    }

    private func filterIncomingArticles(_ articles: Set<Article>) -> Set<Article> {
        // Drop Articles that we can ignore.
        precondition(self.retentionStyle == .syncSystem)
        return Set(articles.filter { !self.articleIsIgnorable($0) })
    }

    private func removeArticles(_ articleIDs: Set<String>, _ database: FMDatabase) {
        deleteRowsWhere(key: DatabaseKey.articleID, equalsAnyValue: Array(articleIDs), in: database)
    }
}

extension Set<ParsedItem> {
    fileprivate func articleIDs() -> Set<String> {
