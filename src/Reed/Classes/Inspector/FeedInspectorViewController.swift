...
final class FeedInspectorViewController: UITableViewController {
    ...
    private var iconImage: IconImage? {
        IconImageCache.shared.imageForFeed(self.feed)
    }

    private var shouldHideHomePageSection: Bool {
        self.feed.homePageURL == nil
    }
    ...
    private lazy var notifyAboutNewArticlesSwitch: UISwitch = {
        let toggle = UISwitch()
        toggle.addTarget(self, action: #selector(self.notifyAboutNewArticlesChanged(_:)), for: .valueChanged)
        return toggle
    }()

    private lazy var alwaysShowReaderViewSwitch: UISwitch = {
        let toggle = UISwitch()
        toggle.addTarget(self, action: #selector(self.alwaysShowReaderViewChanged(_:)), for: .valueChanged)
        return toggle
    }()
    ...
    override func viewDidLoad() {
        ...
        self.tableView.register(InspectorIconHeaderView.self, forHeaderFooterViewReuseIdentifier: "SectionHeader")
        ...
        self.tableView.register(Value1TableViewCell.self, forCellReuseIdentifier: "FolderCell")

        navigationItem.title = self.feed.nameForDisplay
        self.nameTextField.text = self.feed.nameForDisplay

        self.notifyAboutNewArticlesSwitch.setOn(self.feed.isNotifyAboutNewArticles ?? false, animated: false)

        self.alwaysShowReaderViewSwitch.setOn(self.feed.isArticleExtractorAlwaysOn ?? false, animated: false)

        self.homePageLabel.text = self.feed.homePageURL
        self.feedURLLabel.text = self.feed.url
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        if self.nameTextField.text != self.feed.nameForDisplay {
            let nameText = self.nameTextField.text ?? ""
            let newName = nameText
                .isEmpty ? (self.feed.name ?? NSLocalizedString("Untitled", comment: "Feed name")) : nameText
            self.feed.rename(to: newName) { _ in }
        }
    }
    ...
    func feedIconDidBecomeAvailable(_: Notification) {
        self.headerView?.iconView.iconImage = self.iconImage
    }

    @objc
    func notifyAboutNewArticlesChanged(_: Any) {
    ...
    @objc
    func alwaysShowReaderViewChanged(_: Any) {
        self.feed.isArticleExtractorAlwaysOn = self.alwaysShowReaderViewSwitch.isOn
    }
    ...
    override func numberOfSections(in _: UITableView) -> Int {
        self.shouldHideHomePageSection ? 2 : 3
    }

    override func tableView(_: UITableView, numberOfRowsInSection section: Int) -> Int {
        let actualSection = self.actualSectionIndex(for: section)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let actualSection = self.actualSectionIndex(for: indexPath.section)
        ...
        case (0, 1):
            ...
            cell.detailTextLabel?.text = self.feed.notificationDisplayName.capitalized
            cell.accessoryView = self.notifyAboutNewArticlesSwitch
            cell.selectionStyle = .none
            return cell
        ...
        case (0, 2):
            ...
            cell.textLabel?.font = .preferredFont(forTextStyle: .body)
            cell.textLabel?.adjustsFontForContentSizeCategory = true
            cell.accessoryView = self.alwaysShowReaderViewSwitch
            cell.selectionStyle = .none
            return cell
        ...
        case (1, 0):
            ...
            NSLayoutConstraint.activate([
                ...
                self.homePageLabel.trailingAnchor
                    .constraint(equalTo: cell.contentView.layoutMarginsGuide.trailingAnchor),
                self.homePageLabel.topAnchor.constraint(equalTo: cell.contentView.topAnchor, constant: 11),
                self.homePageLabel.bottomAnchor.constraint(equalTo: cell.contentView.bottomAnchor, constant: -11),
            ])
            return cell
        ...
        case (2, 0):
            ...
            NSLayoutConstraint.activate([
                ...
                self.feedURLLabel.trailingAnchor
                    .constraint(equalTo: cell.contentView.layoutMarginsGuide.trailingAnchor),
                self.feedURLLabel.topAnchor.constraint(equalTo: cell.contentView.topAnchor, constant: 11),
                self.feedURLLabel.bottomAnchor.constraint(equalTo: cell.contentView.bottomAnchor, constant: -11),
            ])
            return cell
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            self.headerView = tableView
                .dequeueReusableHeaderFooterView(withIdentifier: "SectionHeader") as? InspectorIconHeaderView
            self.headerView?.iconView.iconImage = self.iconImage
    ...
    private func actualSectionIndex(for displaySection: Int) -> Int {
        if self.shouldHideHomePageSection, displaySection >= 1 {
...
extension FeedInspectorViewController {
    ...
    func notificationUpdateErrorAlert() -> UIAlertController {
        let alert = UIAlertController(
            title: NSLocalizedString("Enable Notifications", comment: "Notifications"),
            message: NSLocalizedString(
                "Notifications need to be enabled in the Settings app.",
                comment: "Notifications need to be enabled in the Settings app."
            ),
            preferredStyle: .alert
        )
        let openSettings = UIAlertAction(
            title: NSLocalizedString("Open Settings", comment: "Open Settings"),
            style: .default
        ) { _ in
