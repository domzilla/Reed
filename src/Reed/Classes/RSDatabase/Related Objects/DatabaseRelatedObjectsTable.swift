...
protocol DatabaseRelatedObjectsTable: DatabaseTable, Sendable {
...
extension DatabaseRelatedObjectsTable {
    ...
    func fetchObjectsWithIDs(_ databaseIDs: Set<String>, in database: FMDatabase) -> [DatabaseObject]? {
        ...
        let fetchedDatabaseObjects = self.objectsWithResultSet(resultSet)
    ...
    func objectsWithResultSet(_ resultSet: FMResultSet) -> [DatabaseObject] {
        resultSet.compactMap(objectWithRow)
    }
