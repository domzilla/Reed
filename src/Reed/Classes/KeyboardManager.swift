...
final class KeyboardManager {
    ...
    init(type: KeyboardType) {
        self._keyCommands = KeyboardManager.globalAuxilaryKeyCommands()

        switch type {
        case .sidebar:
            self._keyCommands.append(contentsOf: KeyboardManager.hardcodeFeedKeyCommands())
        case .timeline, .detail:
            self._keyCommands.append(contentsOf: KeyboardManager.hardcodeArticleKeyCommands())
        default:
            break
        }
        ...
        self._keyCommands.append(contentsOf: globalCommands)
        ...
        self._keyCommands
            .append(contentsOf: specificEntries.compactMap { KeyboardManager.createKeyCommand(keyEntry: $0) })
    }

    static func createKeyCommand(
        title: String,
        action: String,
        input: String,
...
extension KeyboardManager {
    fileprivate static func createKeyCommand(keyEntry: [String: Any]) -> UIKeyCommand? {
        guard let input = createKeyCommandInput(keyEntry: keyEntry) else { return nil }
        let modifiers = self.createKeyModifierFlags(keyEntry: keyEntry)
    ...
    fileprivate static func createKeyCommandInput(keyEntry: [String: Any]) -> String? {
    ...
    fileprivate static func createKeyModifierFlags(keyEntry: [String: Any]) -> UIKeyModifierFlags {
    ...
    fileprivate static func globalAuxilaryKeyCommands() -> [UIKeyCommand] {
        var keys = [UIKeyCommand]()

        let addNewFeedTitle = NSLocalizedString("New Feed", comment: "New Feed")
        keys.append(KeyboardManager.createKeyCommand(
            title: addNewFeedTitle,
            action: "addNewFeed:",
            input: "n",
            modifiers: [.command]
        ))
        ...
        keys.append(KeyboardManager.createKeyCommand(
            title: goToAllUnreadTitle,
            action: "goToAllUnread:",
            input: "2",
            modifiers: [.command]
        ))
        ...
        keys.append(KeyboardManager.createKeyCommand(
            title: toggleReadFeedsFilter,
            action: "toggleReadFeedsFilter:",
            input: "f",
            modifiers: [.command, .shift]
        ))

        let toggleReadArticlesFilter = NSLocalizedString(
            "Toggle Read Articles Filter",
            comment: "Toggle Read Articles Filter"
        )
        keys.append(KeyboardManager.createKeyCommand(
            title: toggleReadArticlesFilter,
            action: "toggleReadArticlesFilter:",
            input: "h",
            modifiers: [.command, .shift]
        ))
    ...
    fileprivate static func hardcodeFeedKeyCommands() -> [UIKeyCommand] {
        var keys = [UIKeyCommand]()
    ...
    fileprivate static func hardcodeArticleKeyCommands() -> [UIKeyCommand] {
        var keys = [UIKeyCommand]()
