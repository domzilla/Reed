...
final class UserNotificationManager {
    ...
    private static let notificationCategory = "NEW_ARTICLE_NOTIFICATION_CATEGORY"
...
extension UserNotificationManager {
    private func sendNotification(feed: Feed, article: Article) {
        ...
        let request = UNNotificationRequest(
            identifier: "articleID:\(article.articleID)",
            content: content,
            trigger: nil
        )
    ...
    /// Determine if there is an available icon for the article. This will then move it to the caches directory and make
    /// it avialble for the notification.
    /// - Parameters:
    ///   - article: `Article`
    ///   - feed: `Feed`
    ...
    private func thumbnailAttachment(for article: Article, feed: Feed) -> UNNotificationAttachment? {
    ...
    private func registerCategoriesAndActions() {
        let readAction = UNNotificationAction(
            identifier: ActionIdentifier.markAsRead,
            title: NSLocalizedString("Mark as Read", comment: "Mark as Read"),
            options: []
        )
        let starredAction = UNNotificationAction(
            identifier: ActionIdentifier.markAsStarred,
            title: NSLocalizedString("Mark as Starred", comment: "Mark as Starred"),
            options: []
        )
        let openAction = UNNotificationAction(
            identifier: ActionIdentifier.openArticle,
