...
enum FeedParser {
    ...
    static func parse(_ parserData: ParserData) throws -> ParsedFeed? {
        ...
        do {
            ...
            case .rss:
                return RDRSSParser.parseFeed(with: parserData).flatMap { ParsedFeedTransformer.parsedFeed($0) }

            case .atom:
                return RDAtomParser.parseFeed(with: parserData).flatMap { ParsedFeedTransformer.parsedFeed($0) }
