...
@interface RDSAXParser ()

@property (nonatomic, weak) id<RDSAXParserDelegate> delegate;
...
@implementation RDSAXParser

+ (void)initialize {

	RDSAXInitLibXMLParser();
}
...
BOOL RDSAXEqualTags(const xmlChar *localName, const char *tag, NSInteger tagLength) {
...
static void startElementSAX(void *context, const xmlChar *localname, const xmlChar *prefix, const xmlChar *URI, int nb_namespaces, const xmlChar **namespaces, int nb_attributes, int nb_defaulted, const xmlChar **attributes) {
	...
	[(__bridge RDSAXParser *)context xmlStartElement:localname prefix:prefix uri:URI numberOfNamespaces:nb_namespaces namespaces:namespaces numberOfAttributes:nb_attributes numberDefaulted:nb_defaulted attributes:attributes];
}


static void	endElementSAX(void *context, const xmlChar *localname, const xmlChar *prefix, const xmlChar *URI) {
	[(__bridge RDSAXParser *)context xmlEndElement:localname prefix:prefix uri:URI];
}


static void	charactersFoundSAX(void *context, const xmlChar *ch, int len) {
	[(__bridge RDSAXParser *)context xmlCharactersFound:ch length:(NSUInteger)len];
}


static void endDocumentSAX(void *context) {
	[(__bridge RDSAXParser *)context xmlEndDocument];
}
...
void RDSAXInitLibXMLParser(void) {
