...
extension DatabaseTable {
    ...
    func selectRowsWhere(key: String, equals value: Any, in database: FMDatabase) -> FMResultSet? {
        database.rd_selectRowsWhereKey(key, equalsValue: value, tableName: name)
    }

    func selectSingleRowWhere(key: String, equals value: Any, in database: FMDatabase) -> FMResultSet? {
        database.rd_selectSingleRowWhereKey(key, equalsValue: value, tableName: name)
    }
    ...
    func selectRowsWhere(key: String, inValues values: [Any], in database: FMDatabase) -> FMResultSet? {
        ...
        return database.rd_selectRowsWhereKey(key, inValues: values, tableName: name)
    }
    ...
    func updateRowsWithDictionary(
        _ dictionary: DatabaseDictionary,
        whereKey: String,
        matches: Any,
        database: FMDatabase
    ) {
        _ = database.rd_updateRows(with: dictionary, whereKey: whereKey, equalsValue: matches, tableName: self.name)
    }

    // MARK: Saving

    func insertRows(_ dictionaries: [DatabaseDictionary], insertType: RDDatabaseInsertType, in database: FMDatabase) {
        for oneDictionary in dictionaries {
            _ = database.rd_insertRow(with: oneDictionary, insertType: insertType, tableName: self.name)
        }
    }

    func insertRow(_ rowDictionary: DatabaseDictionary, insertType: RDDatabaseInsertType, in database: FMDatabase) {
        self.insertRows([rowDictionary], insertType: insertType, in: database)
    }
