...
struct ArticleRenderer {
    ...
    struct Page {
        ...
        init(name: String) {
            self.url = Bundle.main.url(forResource: name, withExtension: "html")!
            self.baseURL = self.url.deletingLastPathComponent()
            self.html = try! String(contentsOfFile: self.url.path, encoding: .utf8)
        }
    }
...
extension ArticleRenderer {
    private var articleHTML: String {
        try! MacroProcessor.renderedText(withTemplate: self.template(), substitutions: self.articleSubstitutions())
    }
    ...
    private var articleCSS: String {
        try! MacroProcessor.renderedText(withTemplate: self.styleString(), substitutions: self.styleSubstitutions())
    }

    fileprivate static var defaultStyleSheet: String = {
        let path = Bundle.main.path(forResource: "stylesheet", ofType: "css")!
        let s = try! String(contentsOfFile: path, encoding: .utf8)
        return "\n\(s)\n"
    }()

    fileprivate static let defaultTemplate: String = {
    ...
    private func styleString() -> String {
        ArticleRenderer.defaultStyleSheet
    }

    private func template() -> String {
        ArticleRenderer.defaultTemplate
    }

    private func articleSubstitutions() -> [String: String] {
        var d = [String: String]()
    ...
    private func byline() -> String {
        ...
        if authors.count == 1, let author = authors.first {
            if author.name == self.article?.feed?.nameForDisplay {
                return ""
            }
        }
    ...
    private func styleSubstitutions() -> [String: String] {
        var d = [String: String]()
