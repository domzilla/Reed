...
extension Data {
    ...
    var isPNG: Bool {
        self.matchesSignature(from: [ImageSignature.png])
    }

    /// Returns `true` if the data begins with a valid GIF signature.
    var isGIF: Bool {
        self.matchesSignature(from: [ImageSignature.gif89a, ImageSignature.gif87a])
    }

    /// Returns `true` if the data begins with a valid JPEG signature.
    var isJPEG: Bool {
        self.matchesSignature(from: [ImageSignature.jpeg])
    }

    /// Returns `true` if the data is an image (PNG, JPEG, or GIF).
    var isImage: Bool {
        self.isPNG || self.isJPEG || self.isGIF
    }
    ...
    var isProbablyHTML: Bool {
        if !self.contains(Search.lessThan) || !self.contains(Search.greaterThan) {
            return false
        }

        // Check for DOCTYPE declaration (strong indicator)
        if
            self.range(of: Search.UTF8.lowercaseDoctype) != nil || self
                .range(of: Search.UTF8.uppercaseDoctype) != nil
        {
            return true
        }

        if
            self.range(of: Search.UTF16.lowercaseDoctype) != nil || self
                .range(of: Search.UTF16.uppercaseDoctype) != nil
        {
            return true
        }

        // Check for html tag (strong indicator)
        if self.range(of: Search.UTF8.lowercaseHTML) != nil || self.range(of: Search.UTF8.uppercaseHTML) != nil {
            return true
        }

        if self.range(of: Search.UTF16.lowercaseHTML) != nil || self.range(of: Search.UTF16.uppercaseHTML) != nil {
            return true
        }

        // Check for head tag (strong indicator)
        if self.range(of: Search.UTF8.lowercaseHead) != nil || self.range(of: Search.UTF8.uppercaseHead) != nil {
            return true
        }

        if self.range(of: Search.UTF16.lowercaseHead) != nil || self.range(of: Search.UTF16.uppercaseHead) != nil {
            return true
        }

        // Check for body tag (good indicator)
        if self.range(of: Search.UTF8.lowercaseBody) != nil || self.range(of: Search.UTF8.uppercaseBody) != nil {
            return true
        }

        if self.range(of: Search.UTF16.lowercaseBody) != nil || self.range(of: Search.UTF16.uppercaseBody) != nil {
            return true
        }
        ...
        let hasCommonHTMLElements = {
            // Check for div tags
            if
                self.range(of: Search.UTF8.lowercaseDiv) != nil || self
                    .range(of: Search.UTF8.uppercaseDiv) != nil ||
                    self.range(of: Search.UTF16.lowercaseDiv) != nil || self
                    .range(of: Search.UTF16.uppercaseDiv) != nil
            {
                return true
            }
            ...
            // Check for p tags
            if
                self.range(of: Search.UTF8.lowercaseP) != nil || self.range(of: Search.UTF8.uppercaseP) != nil ||
                self.range(of: Search.UTF16.lowercaseP) != nil || self.range(of: Search.UTF16.uppercaseP) != nil
            {
                return true
            }
            ...
            // Check for span tags
            if
                self.range(of: Search.UTF8.lowercaseSpan) != nil || self
                    .range(of: Search.UTF8.uppercaseSpan) != nil ||
                    self.range(of: Search.UTF16.lowercaseSpan) != nil || self
                    .range(of: Search.UTF16.uppercaseSpan) != nil
            {
                return true
            }
